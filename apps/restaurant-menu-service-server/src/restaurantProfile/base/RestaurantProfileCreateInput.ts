/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { IsString, IsOptional, ValidateNested } from "class-validator";
import { MenuItemCreateNestedManyWithoutRestaurantProfilesInput } from "./MenuItemCreateNestedManyWithoutRestaurantProfilesInput";
import { Type } from "class-transformer";
import { OrderCreateNestedManyWithoutRestaurantProfilesInput } from "./OrderCreateNestedManyWithoutRestaurantProfilesInput";

@InputType()
class RestaurantProfileCreateInput {
  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  address?: string | null;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  contactInfo?: string | null;

  @ApiProperty({
    required: false,
    type: () => MenuItemCreateNestedManyWithoutRestaurantProfilesInput,
  })
  @ValidateNested()
  @Type(() => MenuItemCreateNestedManyWithoutRestaurantProfilesInput)
  @IsOptional()
  @Field(() => MenuItemCreateNestedManyWithoutRestaurantProfilesInput, {
    nullable: true,
  })
  menuItems?: MenuItemCreateNestedManyWithoutRestaurantProfilesInput;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  name?: string | null;

  @ApiProperty({
    required: false,
    type: () => OrderCreateNestedManyWithoutRestaurantProfilesInput,
  })
  @ValidateNested()
  @Type(() => OrderCreateNestedManyWithoutRestaurantProfilesInput)
  @IsOptional()
  @Field(() => OrderCreateNestedManyWithoutRestaurantProfilesInput, {
    nullable: true,
  })
  orders?: OrderCreateNestedManyWithoutRestaurantProfilesInput;
}

export { RestaurantProfileCreateInput as RestaurantProfileCreateInput };
